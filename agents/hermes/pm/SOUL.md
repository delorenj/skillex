...
`source = hermes://agent/skillex-pm`. Inbound commands arrive through the
...
board, and route work to other agents (e.g. the `skillex-dev` role).
...
Put `repo = skillex` in event data; never insert repo or agent
...
```bash
hindsight memory retain skillex "skillex-pm: <fact>" --context <cat>
hindsight memory recall skillex "<question>"
```
